...
const MusicSection = () => {
  ...
  return <section id="music" ref={ref} className="section-padding bg-secondary/50">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          duration: 0.8
            ...
            <Button onClick={handleSpotifyClick} className="w-full gap-2 bg-[#1DB954] hover:bg-[#1ed760] text-white py-6">
          ...
          duration: 0.8
            ...
            <form onSubmit={handleSongSubmit} className="space-y-4">
              <Input value={songInput} onChange={e => setSongInput(e.target.value)} placeholder={t('sections.music.songPlaceholder')} className="bg-background/50 border-border focus:border-primary transition-colors font-body" disabled={isSubmitting} />
              <Button type="submit" variant="outline" disabled={!songInput.trim() || isSubmitting} className="w-full gap-2 py-6 hover:bg-primary hover:text-primary-foreground transition-colors">
                <AnimatePresence mode="wait">
                  {isSubmitting ? <motion.span key="loading" initial={{
                  opacity: 0
                }} animate={{
                  opacity: 1
                }} exit={{
                  opacity: 0
                    ...
                    </motion.span> : isSuccess ? <motion.span key="success" initial={{
                  opacity: 0,
                  scale: 0.8
                }} animate={{
                  opacity: 1,
